...
# stores information about client-server communication
class Session:
    def __init__(self, port, sequence_num, server_sequence_num, expected_file, expected_flag, checksum):
        self.port = port
        self.sequence_num = sequence_num
        self.server_sequence_num = server_sequence_num
        self.expected_file = expected_file
        self.payload = self.expected_file[:MAX_PAYLOAD_SIZE]
...
class Server:
    ...
    def run(self):
        ...
        while True:
            ...
            # get package from client and client's port
            try:
            ...
            if current_session is None:
                ...
                if sequence_num == 1 and ack_num == 0:
                    ...
                    elif flag == FLAGS["GET_CHK"] and checksum == compute_checksum(package[8:]):
                        try:
                            file = load_file(payload)
                        except IOError:
                            continue
            ...
            if sequence_num == current_session.sequence_num + 1 and ack_num == current_session.server_sequence_num \
                    and flag in current_session.expected_flag:
                if flag == FLAGS["DAT_ACK"]:
                    # all files have been sent ready to end the session
                    if len(current_session.expected_file) <= MAX_PAYLOAD_SIZE:
                        current_session.expected_file = None
                        current_session.payload = None
                        current_session.sequence_num = sequence_num
                        current_session.server_sequence_num += 1
                        current_session.expected_flag = [FLAGS["FIN_ACK"]]
                        self.send_packet(current_session, 0, FLAGS["FIN"])
                    # Continue to send the remaining files
                    else:
                        current_session.expected_file = current_session.expected_file[MAX_PAYLOAD_SIZE:]
                        current_session.payload = current_session.expected_file[:MAX_PAYLOAD_SIZE]
                        current_session.sequence_num = sequence_num
                        current_session.server_sequence_num += 1
                        self.send_packet(current_session, 0, FLAGS["DAT"])
                    ...
                    if checksum == compute_checksum(payload):
                        if len(current_session.expected_file) <= MAX_PAYLOAD_SIZE:
                            ...
                            current_session.server_sequence_num += 1
                            current_session.sequence_num = sequence_num
                            current_session.expected_flag = [FLAGS["FIN_ACK_CHK"]]
